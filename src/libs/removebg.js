...
Module(
  {
    ...
    fromMe: w,
    use: "AI",
    desc: "Removes image background using AI",
  },
  async (message, match) => {
    ...
    if (!RBG_KEY) {
      await message.client.sendMessage(
        message.jid,
        {
          image: await removeBgv2(
            await message.reply_message.download("buffer")
          ),
          caption: "_Use doc command to convert this image to a document_",
        },
...
Module(
  {
    ...
    fromMe: w,
    use: "AI",
    desc: "Enhances/upscales image quality using AI",
  },
  async (message, match) => {
    if (!message.reply_message?.image)
      return await message.send("_Reply to a photo_");
    ...
    try {
      let image = await imageUpscaler(
        await message.reply_message.download("buffer")
      );
      await message.client.sendMessage(
        message.jid,
        { image, caption: "_Enhanced image_" },
        { quoted: message.quoted }
      );
      await message.edit("_Task complete!_", message.jid, removing.key);
    } catch {
