...
const insertOrUpdateUser = async (sock, user) => {
  try {
    const [roleResults] = await db
      .promise()
      .query("SELECT id FROM roles WHERE name = ?", [user.role]);
    ...
    const [userResults] = await db
      .promise()
      .query("SELECT id, phone, lifetime FROM users WHERE phone = ?", [
        user.phone,
      ]);
    ...
    if (userResults.length > 0) {
      ...
      if (
        existingUser.lifetime !== user.lifetime ||
        existingUser.role_id !== roleId
      ) {
        await db
          .promise()
          .query("UPDATE users SET role_id = ?, lifetime = ? WHERE id = ?", [
            roleId,
            user.lifetime,
            existingUser.id,
          ]);
        ...
        if (sock && sock.user) {
          ...
          console.log(
            `Koneksi WhatsApp belum siap. Pesan untuk ${user.phone} akan dikirim nanti.`
          );
        }
      } else {
        console.log(
          `User ${user.phone} sudah ada dengan data yang sama. Tidak ada perubahan yang dilakukan.`
        );
      }
    } else {
